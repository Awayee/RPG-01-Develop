...
public class _lnxAIlibrary : MonoBehaviour {

    //自身重要属性；
    public float Attack;//攻击力
    [Range(0, 100)]
    public float Value;//血量
   ...
   //自身血条显示用到的变量******************************
    ...
    public bool havehinder()
    {
        ...
        return Physics.Raycast(transform.position + Vector3.up * 1.5f, (player.transform.position + Vector3.up * 1.5f) - transform.position, out hit, Vector3.Distance(player.transform.position, transform.position));
    ...
    void instantiatskill1()
    {
        ...
        skill2.GetComponent<enemyskill>().Damage = Attack;//赋予技能攻击力
    ...
    public void selfbeAttack()
    {
        if (beAttack)
        {
            ...
            Value = Value - player.GetComponent<PlayCharacter>().attackPower;
            if (Value<0)
            {
                 Destroy(gameObject);
            }
